...
ID3D11ShaderResourceView*           mercury = NULL;
ID3D11ShaderResourceView*           venus = NULL;
ID3D11ShaderResourceView*           earth = NULL;
ID3D11ShaderResourceView*           mars = NULL;
ID3D11ShaderResourceView*           jupiter = NULL;
ID3D11ShaderResourceView*           saturn = NULL;
ID3D11ShaderResourceView*           uranus = NULL;
ID3D11ShaderResourceView*           neptune = NULL;
...
HRESULT InitDevice()
{
	...
	//hornet.3ds
	//f15.3ds
	Load3DS("earth.3ds", g_pd3dDevice, &g_pVertexBuffer_3ds, &model_vertex_anz);
    ...
    // Load the Texture
    hr = D3DX11CreateShaderResourceViewFromFile( g_pd3dDevice, L"sun.jpg", NULL, NULL, &g_pTextureRV, NULL );
    if( FAILED( hr ) )
        return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"mercury.png", NULL, NULL, &mercury, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"venus.jpg", NULL, NULL, &venus, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"earth.jpg", NULL, NULL, &earth, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"mars.jpg", NULL, NULL, &mars, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"Jupiter.jpg", NULL, NULL, &jupiter, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"saturn.jpg", NULL, NULL, &saturn, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"uranus.png", NULL, NULL, &uranus, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"neptune.jpg", NULL, NULL, &neptune, NULL);
	if (FAILED(hr))
		return hr;
...
UINT offset = 0;
	...
	XMMATRIX S, T, Rs, Ro, M;
	static float angle = 0;
	angle += elapsed / 9000000.0;

	float mercuryorbit = angle*4;
	float venusorbit = angle * 3;
	float earthorbit = angle * 3.5;
	float marsorbit = angle * 2;
	float jupiterorbit = angle;
	float saturnorbit = angle * .75f;
	float uranusorbit = angle * .5f;
	float neptuneoribt = angle * .1f;
	...
	T = XMMatrixTranslation(0, 0, 80);
	Rs = XMMatrixRotationY(angle);
	Ro = XMMatrixRotationY(mercuryorbit);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &mercury);
	g_pImmediateContext->VSSetShaderResources(0, 1, &mercury);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
	...
	Rs = XMMatrixRotationY(angle);
	Ro = XMMatrixRotationY(venusorbit);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &venus);
	g_pImmediateContext->VSSetShaderResources(0, 1, &venus);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
	...
	S = XMMatrixScaling(5, 5, 5);
	T = XMMatrixTranslation(0, 0, 140);
	Ro = XMMatrixRotationY(earthorbit);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &earth);
	g_pImmediateContext->VSSetShaderResources(0, 1, &earth);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
	...
	T = XMMatrixTranslation(0, 0, 160);
	Ro = XMMatrixRotationY(marsorbit);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &mars);
	g_pImmediateContext->VSSetShaderResources(0, 1, &mars);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
	...
	T = XMMatrixTranslation(0, 0, 200);
	Ro = XMMatrixRotationY(jupiterorbit);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &jupiter);
	g_pImmediateContext->VSSetShaderResources(0, 1, &jupiter);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
	...
	S = XMMatrixScaling(8, 8, 8);
	T = XMMatrixTranslation(0, 0, 240);
	Ro = XMMatrixRotationY(saturnorbit);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &saturn);
	g_pImmediateContext->VSSetShaderResources(0, 1, &saturn);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
	...
	S = XMMatrixScaling(6, 6, 6);
	T = XMMatrixTranslation(0, 0, 280);
	Ro = XMMatrixRotationY(uranusorbit);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &uranus);
	g_pImmediateContext->VSSetShaderResources(0, 1, &uranus);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
	...
	S = XMMatrixScaling(5, 5, 5);
	T = XMMatrixTranslation(0, 0, 300);
	Ro = XMMatrixRotationY(neptuneoribt);
	M = S*Rs*T*Ro;
	constantbuffer.World = XMMatrixTranspose(M);
	constantbuffer.View = XMMatrixTranspose(view);
	constantbuffer.Projection = XMMatrixTranspose(g_Projection);
	...
	g_pImmediateContext->VSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBuffer);
	g_pImmediateContext->PSSetShaderResources(0, 1, &neptune);
	g_pImmediateContext->VSSetShaderResources(0, 1, &neptune);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer_3ds, &stride, &offset);
	g_pImmediateContext->PSSetSamplers(0, 1, &g_pSamplerLinear);
	g_pImmediateContext->VSSetSamplers(0, 1, &g_pSamplerLinear);
